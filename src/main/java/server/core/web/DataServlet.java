...
public class DataServlet  extends HttpServlet {
	...
	private String getDatabaseData(String gridID, String keyProperty, String[] clusterIDs, String[] time, 
		...
		if (time.length == 1) {
			...
			return GeoJsonConverter.convertObservations(observations, keyProperty, manager.getGrid(gridID));
			...
			for (int i = 0; i < steps; i++) {
				...
				builder.append(getDatabaseData(gridID, keyProperty, clusterIDs, time, stepsString, req, res));
				if (i < steps - 1) {
					builder.append(", ");
				}
			}
